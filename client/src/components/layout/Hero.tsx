...
import { getHeadingLines, getHeroText, getHeroDescription } from '@/services/settings.service';
...
export default function Hero() {
    ...
    useEffect(() => {
        const fetchHeroSettings = async () => {
            try {
                const [headingLines, heroText, heroDescription] = await Promise.all([
                    getHeadingLines(),
                    getHeroText(),
                    getHeroDescription(),
                ]);
    ...
    useEffect(() => {
        if (typeof gsap === 'undefined' || loading) return;
        ...
        <section className="relative flex min-h-[90vh] flex-col items-center justify-center overflow-hidden bg-white px-4 text-center dark:bg-black">
            ...
            <div className="relative z-10 mx-auto max-w-4xl space-y-8">
                <h1 ref={titleRef} className="text-5xl font-extrabold tracking-tighter text-black sm:text-7xl xl:text-8xl dark:text-white">
                    {(() => {
                        // Use headingLines if available, otherwise use heroText, otherwise fallback to siteConfig
                        if (heroSettings.headingLines) {
                            const lines = heroSettings.headingLines.split(',').map(line => line.trim()).filter(line => line);
                            if (lines.length > 0) {
                                return (
                                    <>
                                        {lines[0]} <br className="hidden sm:block" />
                                        {lines.length > 1 && (
                        ...
                        // Use heroText if available
                        if (heroSettings.heroText) {
                            return heroSettings.heroText;
                        }
                        // Fallback to siteConfig
                        return (
                            <>
                                {siteConfig.hero.title.line1} <br className="hidden sm:block" />
                                <span className="text-transparent bg-clip-text bg-linear-to-r from-zinc-500 to-zinc-900 dark:from-zinc-100 dark:to-zinc-500">
                ...
                <div ref={buttonsRef} className="flex flex-col items-center justify-center gap-4 sm:flex-row sm:gap-6">
                    ...
                    <Link
                        href={siteConfig.hero.buttons.secondary.href}
