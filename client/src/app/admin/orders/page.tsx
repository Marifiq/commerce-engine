...
const statusColors: Record<string, string> = {
  ...
  shipped: "bg-zinc-800 text-white dark:bg-zinc-700 dark:text-zinc-100",
  delivered:
...
export default function OrdersPage() {
  ...
  const handleEditOrder = async (order: Order) => {
    ...
    // Fetch full order details to ensure we have complete product information
    try {
      ...
      const transformedOrder = {
        ...
        total: fullOrder.totalAmount || fullOrder.total || 0,
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-zinc-500 dark:text-zinc-400 text-sm">
          Manage customer fulfillment and track status.
        </p>
      </div>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        {order.status === "pending" && (
                          <button
                            ...
                            className="p-2 text-zinc-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-all cursor-pointer"
                            title="Edit Order"
                          >
                            <Edit2 size={18} />
                          </button>
              ...
              <div>
                ...
                <div className="space-y-4">
                  {selectedOrder.items.map((item: any) => (
                    <div
                      key={item.id}
                      className="flex items-center gap-4 p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-800/50"
                    >
